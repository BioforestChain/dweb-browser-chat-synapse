...
class DWebUserDirectoryHandler(StateDeltasHandler):
    ...
    async def search_users(
        ...
        """
        # users.wallet_address 置换 users.name

        user_name = await self.store.get_user_name_from_wallet_address(search_term)

        if len(user_name) == 0:
            return
        else:
            search_name =  user_name[0][0]

        results = await self.store.search_user_dir(
            user_id, search_name, limit, self.show_locked_users
        )
